...
def get_distance_matrix(data_dir, subject_number, timeslots,
                        ...
                        down_sampling=0):
    if down_sampling != 0:
        total_timeslots = int(math.ceil(timeslots / down_sampling))
        dissimilarity_matrix = np.array([[0.0 for j in range(total_timeslots)]
                                         for i in range(total_timeslots)])
    else:
        dissimilarity_matrix = np.array([[0.0 for j in range(timeslots)]
                                         for i in range(timeslots)])
    barcodes = {}
    increment = 1
    if down_sampling != 0:
        increment = down_sampling
    row = 0
    for i in range(1, timeslots + 1, increment):
        time_1 = f"time_{i}"
        ...
        for j in range(1, i, increment):
            ...
            dissimilarity_matrix[row][column] = distance
            dissimilarity_matrix[column][row] = distance
            column += 1
        row += 1
...
def get_user_input():
    ...
    parser.add_argument('--down', '-r',
                        help='To down sampling (integer < total timeslots)')
...
def main(dataset, method, start_subject=1, end_subject=316,
         distance_calculation='y', mds_calculation='y', down_sampling=0):
    if dataset == 2500:
        # DFC 2500
        ...
        # data_directory = "sample_data/dfc_645_normal"
        distance_matrix_directory = "../dfc_645_down_subjects_distance_matrix_" + method
        mds_directory = "../dfc_645_down_subjects_mds_" + method
    ...
    if mds_calculation == 'y':
        generate_mds(mds_directory, distance_matrix_directory, total_subjects,
                     1, end_subject, distance_method=method)


if __name__ == "__main__":
    get_user_input()
    # data_dir = "sample_data/dfc_645_normal"
    # subject_number = 1
    # timeslots = 754
    # normalize_file_prefix = "normalize_dfc_645_subject_"
    # matrix = get_distance_matrix(data_dir, subject_number, timeslots,
    #                              normalize_file_prefix, distance_method='ws',
    #                              down_sampling=4)
